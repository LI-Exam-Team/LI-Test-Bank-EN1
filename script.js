...
let currentUserInfo = ""; 
let currentCategoryName = ""; // YENİ: Seçilen kategoriyi burada tutacağız
...
function checkLogin() {
    ...
    const loginCard = document.querySelector('#login-screen .card'); // Titreyecek olan kart
    
    // HATA DURUMU FONKSİYONU
    function triggerError(msg) {
        ...
        // 0.5 saniye sonra efekti kaldır (tekrar çalışabilsin diye)
        setTimeout(() => {
...
function generateExam(categoryName) {
    // Kategori ismini hafızaya al (Log için)
    currentCategoryName = categoryName;

    // Dosya ismini küçük harfe çevir (dosyalarımız: questions-easy.json)
...
function copyToClipboard(elementId) {
    ...
    if (elementId === 'questions-box') {
        // GÜNCELLEME: İsim satırına kategori bilgisini de ekledik
